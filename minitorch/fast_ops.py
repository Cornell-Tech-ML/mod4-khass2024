...
def _tensor_matrix_multiply(
    ...
    * Inner loop should have no global writes, 1 multiply.
    ...
    batch_size = max(out_shape[0], 1)
    rows = out_shape[-2]
    cols = out_shape[-1]
    common_dim = a_shape[-1]  # a_shape[-1] == b_shape[-2]
    ...
    for n in prange(batch_size):
        for i in range(rows):
            for j in range(cols):
                ...
                out_idx = (
                    n * out_batch_stride + i * out_strides[-2] + j * out_strides[-1]
                )
                ...
                for k in range(common_dim):
                    temp += (
                        a_storage[a_base + k * a_strides[-1]]
                        * b_storage[b_base + k * b_strides[-2]]
                    )
